    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {activeAccounts.map((account) => {
          ...
          const getIcon = () => {
            if (account.methodName.toLowerCase().includes('easypaisa')) return <Smartphone className="w-5 h-5 text-emerald-600" />;
          ...
          return (
            <div
              ...
              className={`p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 relative ${
                isSelected
                  ? 'bg-emerald-50/80 border-brand-600 shadow-sm'
            ...
            >
              ...
              {isSelected && account.type !== 'cod' && (
                <div className="mt-3 pt-3 border-t border-emerald-200/60 text-xs space-y-1 text-slate-700 bg-white/70 p-2.5 rounded-xl">
                  <p className="font-mono font-bold text-slate-900">
                    Number / Account #: <span className="bg-emerald-100 text-brand-900 px-1.5 py-0.5 rounded">{account.accountNumber}</span>
                  </p>
                  {account.iban && (
